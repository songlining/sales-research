...
**Browser invariant:** Always use `mcp__plugin_superpowers-chrome_chrome__use_browser` for browser work in Copilot. Reuse the user's existing WebMCP-enabled Chrome session. Do **not** manually launch Chrome, start a second isolated-profile Chrome instance, switch to another browser automation tool, or try to connect to Chrome directly from the shell.
...
The underlying browser control channel still uses Chrome's developer tooling. Let the user's WebMCP-enabled Chrome expose that session to the MCP server; do not assume a fixed remote debugging port is available and do not configure one yourself.
...
Scope customer-tree legacy checks to the resolved canonical customer folder, and separately search the repo-level `docs/sales-research/` legacy path for matching artifacts before writing new files. Move any matching legacy artifacts into the unified company folder when possible and continue there to avoid duplicate research. After a repo-level legacy artifact from `docs/sales-research/` is moved or merged successfully, archive, remove, or tombstone the old repo-level artifact immediately so later sessions do not import it again. If a legacy flat brief must be merged manually instead of moved wholesale, archive, remove, or tombstone the old flat brief immediately after the merge succeeds so later sessions do not import it again.
